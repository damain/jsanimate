...
export default class Animate {
    ...
    differentUnitAddition(original, change, firstUnit, secondUnit) {
        ...
        // let result =
        // 	parseFloat(original.split(unit)[0]) + parseFloat(change.split(unit)[0]);
        // return result + unit;
    }
    /**
    ...
    run(options = {}) {
        ...
        this.element.parentElement.style.perspective = this.hasPerspective
            ? this.distance
            : "none";
    ...
    sameUnitAddition(original, change, unit) {
        let result = parseFloat(original.replace(unit, "")) +
            parseFloat(change.replace(unit, ""));
        return result + unit;
    }
    scale({ x = 1, y = 1, add = false }) {
        ...
        if (typeof arguments[0] === "object") {
            if (arguments[0].hasOwnProperty("x") && !arguments[0].hasOwnProperty("y"))
                y = x;
            if (arguments[0].hasOwnProperty("y") && !arguments[0].hasOwnProperty("x"))
                x = y;
        }
        add
            ? (this.size = { x: this.size.x + x, y: this.size.y + y })
            : (this.size = { x, y });
        return this;
    }
    /**
    ...
    translate({ x, y, add = false, }) {
        ...
        const addCords = () => {
            this.cords = {
                x: this.computeChange(this.cords.x, x),
                y: this.computeChange(this.cords.y, y),
            };
        };
        const setCords = () => {
            this.cords = { x, y };
        };
        add ? addCords() : setCords();
        return this;
    }
    /**
     * 3D rotation along the x axis
...
const getDefaultOptions = (options) => {
    const { composite = "replace", delay = 0, direction = "normal", duration = 1000, easing = "cubic-bezier(0.34, 1, 0.64, 1)", endDelay = 0, fill = "forwards", iterationStart = 0, iterations = 1, iterationComposite = "replace", } = options || {};
    return Object.assign(Object.assign({}, options), { delay,
        direction,
        duration,
        easing,
        endDelay,
