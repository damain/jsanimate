...
export default class Animate {
	...
	cords: { x: number | string; y: number | string };
	cords3D: { x: number | string; y: number | string; z: number | string };
	angle: number;
	 ...
	 * @returns instance of the class
	 */
	run(options: Partial<KeyframeAnimationOptions> = {}) {
	...
	sameUnitAddition(original, change, unit) {
		let result =
			parseFloat(original.replace(unit, "")) +
...
): Partial<KeyframeAnimationOptions> => {
	const {
		composite = "replace",
		delay = 0,
		direction = "normal",
 ...
 */
export const selector = (name: string | HTMLElement) => {
